...
export default function CreateListing() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      //Condition to restrict user from submiting empty listing
      if (formData.imageUrls.length < 1) return setError('You must upload at least one image')
      // Condition for Price of listing
      if (+formData.regularPrice < +formData.discountPrice) return setError('Discount Price must be lower than regular price')
      // Add your code here to submit the form data to the database
      setLoading(true);
      setError(false);
    ...
    <main className="p-3 max-w-4xl mx-auto border bg-slate-50 mt-2">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <button  disabled={uploading || uploading} className="bg-slate-700 p-3 rounded-lg text-white hover:opacity-95 uppercase disabled:opacity-85 mt-2">
